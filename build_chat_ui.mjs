...
if (!fs.existsSync("src/components/chat")) {
  fs.mkdirSync("src/components/chat", { recursive: true });
}
...
`;
fs.writeFileSync("src/components/chat/GlobalChat.tsx", chatCode.trim());

// 3. Inject into Header.tsx
let header = fs.readFileSync("src/components/layout/Header.tsx", "utf-8");
if (!header.includes("toggleChat")) {
  header = header.replace(
    "const { setSidebarOpen } = useUIStore();",
    "const { setSidebarOpen, toggleChat } = useUIStore();"
  );
  header = header.replace(
    "import { Menu, Plus, Zap } from \x27lucide-react\x27;",
    "import { Menu, Plus, Zap, MessageCircle } from \x27lucide-react\x27;"
  );
  
  // Inject chat button next to notification bell
  header = header.replace(
    "<NotificationBell />",
    `<button onClick={toggleChat} className="relative p-2 rounded-full hover:bg-white/10 transition-colors hidden sm:flex">
...
// 4. Inject GlobalChat into App.tsx layout
let app = fs.readFileSync("src/App.tsx", "utf-8");
if (!app.includes("GlobalChat")) {
  app = app.replace(
    "import { Sidebar } from \x27@/components/layout/Sidebar\x27;",
    "import { Sidebar } from \x27@/components/layout/Sidebar\x27;\nimport { GlobalChat } from \x27@/components/chat/GlobalChat\x27;"
  );
  app = app.replace(
    "<Sidebar />",
    "<Sidebar />\n        <GlobalChat />"
