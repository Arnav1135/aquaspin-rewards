...
export function LiveWinnersMarquee() {
  ...
  return (
    <div className="w-full bg-[#0a0f1d] border-y border-white/5 overflow-hidden flex items-center h-10 relative z-40">
...
fs.writeFileSync("src/components/ui/LiveWinnersMarquee.tsx", marqueeCode.trim());
console.log("Marquee built.");

let app = fs.readFileSync("src/App.tsx", "utf-8");
if (!app.includes("LiveWinnersMarquee")) {
  app = app.replace(
    "import { LevelUpManager } from \x27@/components/ui/LevelUpManager\x27;",
    "import { LevelUpManager } from \x27@/components/ui/LevelUpManager\x27;\nimport { LiveWinnersMarquee } from \x27@/components/ui/LiveWinnersMarquee\x27;"
  );
  app = app.replace(
    "<LevelUpManager />",
    "<LevelUpManager />\n        <LiveWinnersMarquee />"
