...
export class CandyRenderer {
  ...
  /** Preload all candy textures once. */
  ...
  static getTexture(color, type = CANDY_TYPES.NORMAL) {
    return CandyRenderer._textures[CandyRenderer._key(color, type)] || PIXI.Texture.WHITE;
  }

  static render(color, type = CANDY_TYPES.NORMAL) {
    const key = CandyRenderer._key(color, type);
    return CandyRenderer._cache[key]
      ? CandyRenderer._addGlow(CandyRenderer._cache[key], color)
      : CandyRenderer._inlineFallback(color, type);
  }
  ...
  static renderTimer(color, countdown) {
    const timerOverlay = `
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:5;">
        <circle cx="100" cy="120" r="38" fill="rgba(20,10,30,0.88)" stroke="white" stroke-width="3"/>
        <text x="100" y="132" font-size="38" font-weight="900" fill="${countdown <= 3 ? '#ff2244' : '#ffe066'}" font-family="Fredoka One,sans-serif" text-anchor="middle">${countdown}</text>
      </svg>`;
  ...
  static renderBlocker(type, strength) {
    switch (type) {
      case BLOCKER_TYPES.FROSTING: {
        ...
        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" style="width:100%;height:100%;display:block;"><ellipse cx="100" cy="180" rx="55" ry="12" fill="rgba(0,0,0,0.2)"/><rect x="25" y="25" width="150" height="150" rx="28" fill="${fill}" stroke="${stroke}" stroke-width="8"/><path d="M45,100 Q100,30 155,100 Q100,170 45,100 Z" fill="rgba(255,255,255,0.55)"/><circle cx="100" cy="100" r="20" fill="#e91e63"/></svg>`;
      }
      case BLOCKER_TYPES.CHOCOLATE:
        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" style="width:100%;height:100%;display:block;"><rect x="18" y="18" width="164" height="164" rx="18" fill="#4e2f1d" stroke="#27100a" stroke-width="6"/><rect x="30" y="30" width="62" height="62" rx="8" fill="#5d3a24"/><rect x="108" y="30" width="62" height="62" rx="8" fill="#5d3a24"/><rect x="30" y="108" width="62" height="62" rx="8" fill="#5d3a24"/><rect x="108" y="108" width="62" height="62" rx="8" fill="#5d3a24"/></svg>`;
      case BLOCKER_TYPES.LICORICE:
        return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" style="width:100%;height:100%;display:block;"><circle cx="100" cy="100" r="82" fill="#1a1a1a" stroke="#000" stroke-width="6"/><circle cx="100" cy="100" r="60" fill="none" stroke="#333" stroke-width="8"/><circle cx="100" cy="100" r="38" fill="none" stroke="#4d4d4d" stroke-width="7"/></svg>`;
      default:
  ...
  static renderLayerOverlay(layerType, value) {
    ...
    switch (layerType) {
      ...
      case LAYER_TYPES.CHAIN:
        return `<svg viewBox="0 0 100 100" style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:4;"><path d="M10,10 L90,90 M90,10 L10,90" stroke="#78909c" stroke-width="8" stroke-linecap="round" opacity="0.85"/><path d="M10,10 L90,90 M90,10 L10,90" stroke="#b0bec5" stroke-width="3" stroke-linecap="round" stroke-dasharray="8,6" opacity="0.95"/></svg>`;
      case LAYER_TYPES.LOCK:
        return `<svg viewBox="0 0 100 100" style="position:absolute;inset:0;width:100%;height:100%;pointer-events:none;z-index:4;"><rect x="35" y="45" width="30" height="30" rx="6" fill="#ffd54f" stroke="#ffb300" stroke-width="3"/><circle cx="50" cy="60" r="4" fill="#3e2723"/><path d="M40,45 L40,30 A10,10 0 0,1 60,30 L60,45" fill="none" stroke="#cfd8dc" stroke-width="4"/></svg>`;
  ...
  static _inlineFallback(color, type) {
    ...
    const id = `fb_${color}_${type}`;
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" style="width:100%;height:100%;display:block;filter:drop-shadow(0 4px 8px ${p.glow})"><defs><radialGradient id="fbg_${id}" cx="35%" cy="30%" r="70%"><stop offset="0%" stop-color="${p.light}"/><stop offset="50%" stop-color="${p.primary}"/><stop offset="100%" stop-color="${p.dark}"/></radialGradient></defs><circle cx="100" cy="100" r="78" fill="url(#fbg_${id})" stroke="${p.mid}" stroke-width="3"/><circle cx="128" cy="62" r="10" fill="white" opacity="0.8"/></svg>`;
  }
}
