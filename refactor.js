...
for (const file of files) {
  ...
  if (content.includes('GameEngine3D')) {
    ...
    if (content.includes('<GameEngine3D')) {
      ...
      // we must close <Physics> if it was opened.
      // we can check if <Physics> is in the file, but we should be careful.
      const closeTagRegex = /<\/GameEngine3D>/g;
      content = content.replace(closeTagRegex, (match) => {
         // if there's <Physics> opened in the same block, we need to close it.
         // Let's do a simple heuristic: if enablePhysics={true} was present.
