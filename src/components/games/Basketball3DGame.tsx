...
function Ball({ shooting, power, onResult }: { shooting: boolean; power: number; onResult: (scored: boolean) => void }) {
  ...
  const y = shooting ? 0.35 + 1.2 * t + 4.5 * t * (1 - t) + (power - 0.65) * 0.7 * t : 0.35;
  return <mesh position={[0, y, z]} rotation={[ref.t * 6, ref.t * 4, ref.t * 3]} castShadow>
...
function Court() {
  return <>
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.05, 0]} receiveShadow><planeGeometry args={[8.5, 10]} /><meshStandardMaterial color="#8d542c" roughness={0.4} /></mesh>
    <mesh position={[0, 2.8, -3.25]} castShadow><boxGeometry args={[3.1, 2.2, 0.12]} /><meshStandardMaterial color="#dff7ff" transparent opacity={0.8} roughness={0.15} metalness={0.35} /></mesh>
    <mesh position={[0, 1.55, -2.92]} castShadow><torusGeometry args={[0.62, 0.075, 16, 48]} /><meshStandardMaterial color="#ff8a3d" emissive="#8b3210" emissiveIntensity={0.7} metalness={0.65} roughness={0.2} /></mesh>
    {Array.from({ length: 7 }).map((_, i) => <mesh key={i} position={[-1.1 + i * 0.36, 0.9, -2.92]}><cylinderGeometry args={[0.012, 0.012, 1.1, 8]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.7} /></mesh>)}
    <RoundedBox args={[2.4, 0.25, 0.45]} radius={0.08} position={[0, 1.1, -2.9]}><meshStandardMaterial color="#1d2733" metalness={0.8} roughness={0.2} /></RoundedBox>
...
export function Basketball3DGame({ onClose }: { onClose: () => void }) {
  ...
  const takeShot = () => { if (shooting) return; setResult(null); setShooting(true); setShot(s => s + 1); };
  const finished = (scored: boolean) => { if (scored) setScore(s => s + 1); setResult(scored ? 'score' : 'miss'); setShooting(false); };
  return <GameFrame title="3D Basketball Arcade" onClose={onClose} score={score} onRestart={() => { setScore(0); setShooting(false); setResult(null); setShot(s => s + 1); }}>
    <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_20%,#6d351b,#120b08_70%)]">
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 w-[min(92%,460px)] rounded-2xl border border-white/15 bg-black/35 p-4 backdrop-blur-xl">
        ...
        {result && <div className="mt-2 text-center text-xs font-black text-white/80">{result === 'score' ? '🏀 SWISH!' : 'MISS — adjust your power'}</div>}
      </div>
    </div>
  </GameFrame>;
}
