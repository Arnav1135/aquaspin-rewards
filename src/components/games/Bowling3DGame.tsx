...
function Pin({ position, fallen, index }: { position: [number, number, number]; fallen: boolean; index: number }) {
  ...
  useFrame((_, dt) => {
    if (fallen) ref.t = Math.min(1, ref.t + dt * 3.8);
    else ref.t = 0;
  });
  ...
  return (
    ...
    >
      <mesh castShadow position={[0, 0.62, 0]}>
        <cylinderGeometry args={[0.13, 0.24, 1.1, 28]} />
        <meshStandardMaterial color="#f5f7fb" roughness={0.22} metalness={0.04} envMapIntensity={1.1} />
      </mesh>
      ...
      <mesh position={[0, 0.98, 0]}>
        ...
        <meshStandardMaterial color="#e94b4b" emissive="#7d1111" emissiveIntensity={0.15} roughness={0.28} />
      </mesh>
    </group>
  );
}
...
function Lane({ pinsFallen, rolling, power, spin }: { pinsFallen: boolean; rolling: boolean; power: number; spin: number }) {
  ...
  useFrame((_, dt) => {
    if (rolling) ballRef.t = Math.min(1, ballRef.t + dt * (0.48 + power * 0.45));
    else ballRef.t = 0;
  });
  ...
  return (
    <>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.08, -1.4]} receiveShadow>
        ...
        <meshStandardMaterial color="#b97943" roughness={0.3} metalness={0.03} />
      </mesh>
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.06, -1.4]}>
        <planeGeometry args={[4.8, 12.5]} />
        <meshStandardMaterial color="#d39a5e" roughness={0.27} />
      </mesh>
      {[-2.65, 2.65].map(xp => (
        <RoundedBox key={xp} args={[0.12, 0.32, 13]} radius={0.02} position={[xp, 0.15, -1.4]}>
          <meshStandardMaterial color="#e5edf3" metalness={0.62} roughness={0.2} />
        </RoundedBox>
      ))}
    ...
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_10%,#263d63,#070b12_72%)]">
        ...
        >
          ...
          <Text position={[0, 0.12, 3.1]} rotation={[-Math.PI / 2, 0, 0]} fontSize={0.5} color="#e7f7ff">
            AQUA SPIN BOWLING
          </Text>
        ...
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 w-[min(92%,480px)] rounded-2xl border border-white/15 bg-black/35 p-4 backdrop-blur-xl">
          ...
          <button
            onClick={roll}
            disabled={rolling}
            className="mt-4 w-full rounded-xl bg-gradient-to-r from-cyan-400 to-violet-500 px-5 py-3 font-black text-slate-950 transition-transform hover:scale-[1.01] disabled:opacity-50"
          >
