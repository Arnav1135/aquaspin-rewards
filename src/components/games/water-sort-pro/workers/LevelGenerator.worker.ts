...
self.onmessage = (e: MessageEvent) => {
  ...
  if (type === 'GENERATE') {
    const { targetDifficulty, colorCount, tubeCount, tubeCapacity } = payload;
    try {
      const levelDef = LevelGenerator.generate(targetDifficulty, colorCount, tubeCount, tubeCapacity);
